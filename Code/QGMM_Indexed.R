...
estimationDataFileName <- file.path(base.path, "rafael/csv_output/unseasonal_consumption/estimation_data_weekly_4w.csv")
...
for (ii in 1:length(rateTypes)) {
  ...
  for (jj in 1:nrow(dateSplits)) {
    ...
    outputFileName <- file.path(base.path, paste0("EIS/Output/Phase2/QGMM_Weekly_4w_", rateVar, "_seasonal.adjust_", timePeriod, ".csv"))
