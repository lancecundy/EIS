...
for (ii in 1:length(rateTypes)) {
  ...
  for (jj in 1:nrow(dateSplits)) {
    ...
    seasonality.Y <- lm(Y ~ I(MONTH), data=estimationDataSubset)
    Y.seasoned <- seasonality.Y$res
    
    seasonality.Z1 <- lm(Z1 ~ MONTH, data=estimationDataSubset)
    Z1.seasoned <- seasonality.Z1$res
